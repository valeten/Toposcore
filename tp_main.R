...
res_surv <- load_or_compute('res_surv.rds',
...
res <- load_or_compute('res_pairs.rds', screen_pairs(met4_disc_filt))
# filter based on Fisher Bonferroni-corrected p-values <= 0.05
res_filt <- load_or_compute('res_pairs_filt.rds', {
...
SIGB1 <- load_or_compute('sigb1.rds', c(cc_species$C5, cc_species$C6))
SIGB2 <- load_or_compute('sigb2.rds', c(cc_species$C1, cc_species$C2, cc_species$C3))
...
### 3.3 RCC+NSCLC analysis set
log_msg('####### RCC+NSCLC analysis set #########')
clin_rcc_nsclc <- load_clin(cohort = 'RCC-NSCLC')
met4_rcc_nsclc <- load_microbiome(clin_rcc_nsclc)
scores_rcc_nsclc <- compute_toposcore(met4_rcc_nsclc, sigb1 = SIGB1, sigb2 = SIGB2) 
pred_rcc_nsclc <- clin_rcc_nsclc %>% left_join(scores_rcc_nsclc, by = 'Sample_id') %>% assign_prediction(ycut_r$THRESHOLD, ycut_nr$THRESHOLD)
hr_rcc_nsclc <- get_hr(pred_rcc_nsclc, type = 'OS', by = 'PRED')
log_msg('Prediction RCC+NSCLC: HR = %.2f [%.2f-%.2f], p = %.1e', hr_rcc_nsclc[1], hr_rcc_nsclc[2], hr_rcc_nsclc[3], hr_rcc_nsclc[4])
fig_km_rcc_nsclc <- print_plot(plot_mykm(pred_rcc_nsclc, type = 'OS', by = 'PRED'))
ggsave(fig_km_rcc_nsclc, filename = 'fig_km_rcc_nsclc.pdf', width = 10, height = 10, units = "cm")
scores_rcc_nsclc_short <- compute_toposcore(met4_rcc_nsclc, sigb1 = SIGB1_PCR, sigb2 = SIGB2_PCR)
pred_rcc_nsclc_short <- clin_rcc_nsclc %>% left_join(scores_rcc_nsclc_short, by = 'Sample_id') %>% assign_prediction(ycut_r$THRESHOLD, ycut_nr$THRESHOLD)
hr_rcc_nsclc_short <- get_hr(pred_rcc_nsclc_short, type = 'OS', by = 'PRED')
log_msg('Prediction RCC+NSCLC (short): HR = %.2f [%.2f-%.2f], p = %.1e', hr_rcc_nsclc_short[1], hr_rcc_nsclc_short[2], hr_rcc_nsclc_short[3], hr_rcc_nsclc_short[4])
fig_km_rcc_nsclc_short <- print_plot(plot_mykm(pred_rcc_nsclc_short, type = 'OS', by = 'PRED'))
ggsave(fig_km_rcc_nsclc_short, filename = 'fig_km_rcc_nsclc_short.pdf', width = 10, height = 10, units = "cm")
